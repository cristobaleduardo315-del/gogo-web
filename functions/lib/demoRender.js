...
// menú/catálogo de muestra en minutos, y su vista previa resultante. Ver
// migrations/0009_demo_leads.sql y functions/lib/demoData.js.
//
...
const WIZARD_STYLES = `
  :root{ --lime:#ccff00; --lime-text:#4a6300; --bg:#f4f4f2; --panel:#ffffff; --border:#e6e6e1; --text:#0b0b0b; --text-dim:#6b6b6b; --red:#c23a3a; }
  *{box-sizing:border-box;}
  ...
  .wz-top{display:flex;align-items:center;justify-content:center;padding:22px 16px 0;}
  .wz-top img{height:26px;}
  .wz-wrap{max-width:560px;margin:0 auto;padding:24px 16px 60px;}
  .wz-card{background:var(--panel);border:1px solid var(--border);border-radius:18px;padding:28px 24px;box-shadow:0 10px 30px rgba(0,0,0,0.05);}
  .wz-progress{display:flex;gap:6px;margin-bottom:22px;}
  .wz-progress span{flex:1;height:4px;border-radius:2px;background:var(--border);}
  .wz-progress span.done{background:var(--lime);}
  .wz-step{display:none;}
  .wz-step.active{display:block;}
  .wz-step h2{font-size:20px;margin:0 0 6px;}
  .wz-step p.sub{color:var(--text-dim);font-size:13.5px;margin:0 0 20px;}
  .wz-field{margin-bottom:16px;}
  .wz-field label{display:block;font-size:13px;font-weight:700;margin-bottom:6px;}
  .wz-field input[type=text],.wz-field input[type=number],.wz-field input[type=tel],.wz-field input[type=email],.wz-field input[type=url],.wz-field textarea{
    width:100%;padding:11px 12px;border:1px solid var(--border);border-radius:10px;font-size:14px;font-family:inherit;background:#fafaf9;
  }
  .wz-field textarea{resize:vertical;min-height:64px;}
  .wz-field .hint{font-size:11.5px;color:var(--text-dim);margin-top:5px;}
  .wz-type-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:6px;}
  .wz-type-card{border:2px solid var(--border);border-radius:14px;padding:20px 14px;text-align:center;cursor:pointer;background:#fafaf9;transition:border-color 0.15s,background 0.15s;}
  .wz-type-card .ic{font-size:26px;display:block;margin-bottom:8px;}
  .wz-type-card strong{display:block;font-size:14px;}
  .wz-type-card.selected{border-color:var(--lime-text);background:var(--lime);}
  .wz-product{border:1px solid var(--border);border-radius:12px;padding:14px;margin-bottom:14px;background:#fafaf9;position:relative;}
  .wz-product .rm{position:absolute;top:10px;right:10px;background:none;border:none;color:var(--text-dim);cursor:pointer;font-size:13px;}
  .wz-add-product{background:none;border:1px dashed var(--border);border-radius:10px;padding:10px;width:100%;font-size:13px;font-weight:700;color:var(--text-dim);cursor:pointer;}
  .wz-photo-row{display:flex;align-items:center;gap:12px;margin-top:4px;}
  .wz-photo-preview{width:52px;height:52px;border-radius:10px;object-fit:cover;background:var(--border);flex-shrink:0;}
  ...
  .btn-primary{background:var(--lime);color:#0b0b0b;}
  .btn-primary:disabled{opacity:0.5;cursor:default;}
  .btn-ghost{background:transparent;color:var(--text);}
  .wz-error{background:#fdecec;color:var(--red);border-radius:10px;padding:10px 12px;font-size:13px;margin-bottom:16px;display:none;}
  .wz-error.show{display:block;}
  .wz-loading{display:none;text-align:center;padding:60px 16px;}
  .wz-loading.show{display:block;}
  .wz-spinner{width:34px;height:34px;border-radius:50%;border:3px solid var(--border);border-top-color:var(--lime-text);animation:wzspin 0.8s linear infinite;margin:0 auto 16px;}
...
<body>
  <div class="wz-top"><img src="/assets/logo.webp" alt="gogo"></div>
  <div class="wz-wrap">
    <div class="wz-card">
      ...
      <form id="wzForm">
        <div class="wz-step" data-step="0">
          ...
          <div class="wz-type-grid">
            <div class="wz-type-card" data-type="restaurante"><span class="ic">🍔</span><strong>Restaurante</strong></div>
            <div class="wz-type-card" data-type="tienda"><span class="ic">🛍️</span><strong>Tienda online</strong></div>
          </div>
        </div>
        ...
        <div class="wz-step" data-step="5">
          ...
          <div class="wz-field"><label>Instagram (opcional)</label><input type="url" id="fInstagram" placeholder="https://instagram.com/tunegocio"></div>
          <div class="wz-field"><label>TikTok (opcional)</label><input type="url" id="fTiktok" placeholder="https://tiktok.com/@tunegocio"></div>
...
(function () {
  ...
  function collectPayload() {
    ...
    return {
      ...
      instagramUrl: document.getElementById('fInstagram').value.trim(),
      tiktokUrl: document.getElementById('fTiktok').value.trim(),
    };
  }
...
// ---------------------------------------------------------------------
// Vista previa del resultado (functions/demo/vista/[id].js)
// ---------------------------------------------------------------------
...
const SKINS = {
  restaurante: { theme: "#c2410c", dark: "#9a3412", cream: "#fdf3e7", verb: "Ver menú", noun: "producto" },
  tienda: { theme: "#3d4eac", dark: "#2f3d8a", cream: "#f4f4f2", verb: "Ver catálogo", noun: "producto" },
};
...
export function renderDemoPreview({ lead, categories, products }, activateUrl) {
  const skin = SKINS[lead.business_type] || SKINS.restaurante;
...
<style>
  *{box-sizing:border-box;margin:0;padding:0;}
  html,body{background:${skin.cream};}
  ...
  .dp-hero{background:${skin.theme};color:#fff;padding:44px 20px 34px;text-align:center;}
  .dp-hero img.logo{width:min(180px,60%);height:auto;margin:0 auto 14px;display:block;border-radius:16px;}
  .dp-hero h1{font-size:clamp(22px,6vw,30px);margin-bottom:6px;}
  .dp-hero .type{font-size:12px;letter-spacing:0.14em;text-transform:uppercase;opacity:0.85;margin-bottom:24px;}
  .dp-cta-list{display:flex;flex-direction:column;gap:10px;max-width:360px;margin:0 auto;}
  ...
  .dp-cta-main{background:#fff;color:${skin.theme};}
  .dp-cta-alt{background:transparent;color:#fff;border:2px solid rgba(255,255,255,0.7);}
  .dp-body{max-width:760px;margin:0 auto;padding:36px 18px 0;}
  .dp-cat{margin-bottom:32px;}
  .dp-cat h3{font-size:18px;margin-bottom:14px;color:${skin.dark};}
  ...
  .dp-item img,.dp-noimg{width:100%;height:140px;object-fit:cover;background:${skin.cream};display:block;}
  ...
  .dp-item-top span{font-size:13px;font-weight:700;color:${skin.dark};white-space:nowrap;}
...
<body>
  ...
  <div class="dp-hero">
    ...
    <div class="type">${lead.business_type === "tienda" ? "Tienda online" : "Restaurante"}</div>
    <div class="dp-cta-list">
      <a class="dp-cta dp-cta-main" href="#productos">${skin.verb}</a>
  ...
  <div class="dp-body" id="productos">
