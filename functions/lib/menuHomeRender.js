...
// original.
//
// Esta lógica vive en lib/ (en vez de directo en functions/menu/[slug].js)
// para poder reusarla desde dos rutas: la ruta normal /menu/:slug Y el
// middleware raíz que sirve el mismo contenido cuando el negocio tiene su
// propio dominio (ej. www.westburger.com) — ver functions/_middleware.js.
import { escapeHtml } from "./layout.js";
import { loadPublicMenu } from "./menuData.js";
import { waLink, mapsLink, FONT_LINKS, themeVars, rootCss, notFoundPage } from "./menuTheme.js";
...
export async function renderMenuHome(env, slug) {
  const data = await loadPublicMenu(env.DB, String(slug || "").toLowerCase());
